...
import com.hoogle.fileditto.Utils.ItemFix;
import com.hoogle.fileditto.Utils.RecyclerViewArrayAdapter;
...
import static com.hoogle.fileditto.Utils.Utils.getRandomColor;
import static com.hoogle.fileditto.Utils.Utils.isOreoOrAbove;
import static com.hoogle.fileditto.sender.DittoActivity.ShareUIHandler.LIST_API_CLIENTS;
import static com.hoogle.fileditto.sender.DittoActivity.ShareUIHandler.UPDATE_AP_STATUS;
...
public class DittoActivity extends AppCompatActivity {
        ...
        m_sender_ap_switch_listener = new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    ...
                    showMessageDialogWithListner(getString(R.string.p2p_receiver_close_warning), new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            Log.d(TAG, "sending service  stop ");
                            resetSenderUi(true);
                        }
                    }, true, false);
                }
    ...
    protected void onResume() {
        ...
        if (Utils.isShareServiceRunning(getApplicationContext())) {
            if (!m_apControlSwitch.isChecked()) {
                Log.e(TAG, "service running");//, changing switch status and start handler for ui changes
                changeApControlCheckedStatus(true);
            }
            refreshApData();
    ...
    public void showMessageDialogWithListner(String message,
        ...
        builder.setPositiveButton(getString(R.string.ActionOk), listner);
        if (showNegavtive)
            builder.setNegativeButton(getString(R.string.Actioncancel),
    ...
    private void updateReceiverListItem(String ip, int progress, String updatetext, String fileName) {
        ...
        if (null != taskListItem) {
            ...
            Log.e(TAG, "no files to transfer******");
        } }
